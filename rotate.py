...
# Rotation matrix function around y-axis 
# Checking the molecule, it is aligned with the y-axis
def rotate_matrix (x, y, z, angle, x_shift=0, y_shift=0, z_shift=0, units="DEGREES"):
    """
...
def test_Cn(x1,y1,z1,x2,y2,z2,x3,y3,z3):
    ...
    for angle in angles:
        ...
        coord1_r=rotate_matrix(x1,y1,z1,angle)
        coord2_r=rotate_matrix(x2,y2,z2,angle)
        coord3_r=rotate_matrix(x3,y3,z3,angle)
        #rename the elements
        x1_r=coord1_r[0]
        y1_r=coord1_r[1]
        z1_r=coord1_r[2]
    ...
    if higher!=1:
        print(f"The higher rotation symmetry operation is C{higher}")
    else:
